...
::: lead
Part II를 여기까지 오는 동안 "미정의 동작"이라는 말을 이미 여섯 번 넘게 만났다 — 그런데 이 책은 그 말을 한 번도 정면으로 정의한 적이 없다. 늘 "이건 UB다, 자세한 건 나중에"로 미뤄 왔다. 이 절이 그 빚을 갚는다. 표준이 UB를 정확히 뭐라고 규정하는지, 그 규정이 컴파일러의 최적화 결정에 왜 직접 연결되는지를 이 환경에서 실제로 재현해서 보여준다. 그다음 지금까지 산발적으로 써 온 `-fsanitize=address`와 `-fsanitize=undefined`를 정식으로 정리한다 — 각각 무엇을 잡고 무엇을 못 잡는지, 켰을 때 실제로 얼마나 느려지는지, 그리고 왜 이 도구를 로봇에 실어 보내면 안 되는지까지.
:::
...
| 등급 | 표준의 태도 | 실측/예 |
| --- | --- | --- |
| 정의된 동작 (defined behavior) | 결과를 정확히 못박는다 | unsigned의 랩어라운드는 모듈로 $2^{32}$로 정의된다 — `0u - 1`은 항상 `4294967295`다([1.2](#/types) 실측) |
| 미지정 동작 (unspecified behavior) | 여러 결과 중 하나가 나오지만 어느 것인지는 표준이 정하지 않고, 구현이 그 선택을 문서화할 의무도 없다 | 함수 인자의 평가 순서 — [1.5](#/control-flow)에서 같은 `f(i++, i++)`가 g++와 clang에서 실제로 다르게 나왔다 |
| 구현 정의 동작 (implementation-defined behavior) | 여러 결과 중 하나를 구현이 고르되, **그 구현은 어느 쪽을 골랐는지 문서로 못박아야 한다** | `sizeof(int)` — 이 환경(LP64)은 4([1.2](#/types) 실측), Windows(LLP64)는 다른 배치를 문서화한다. `char`의 부호도 마찬가지다(x86 signed, ARM unsigned) |
| 미정의 동작 (undefined behavior, UB) | 결과에 **아무 요구도 하지 않는다** — 표준을 위반하지 않고도 무슨 일이든 일어날 수 있다 | signed 오버플로([1.2](#/types)), 댕글링 포인터 역참조([2.2](#/pointers)) |
...
`x` 값을 아예 안 본다. 비교 명령(`cmp`)도, 분기 명령(`je`/`jne`)도 없다 — **`if`가 통째로 사라지고 "정상 범위로 판정" 쪽으로 직행하는 코드만 남았다.** 컴파일러가 "`x + 1 > x`는 signed 오버플로가 없다고 가정하면 항상 참"이라고 증명해 버렸기 때문이다. `x`가 어떤 값을 갖고 들어오든, 오버플로가 UB인 이상 오버플로가 나는 입력은 "일어나지 않는 경우"로 취급되고, 남는 경우에서는 이 식이 항상 참이다.
...
`-fwrapv`가 무오버플로 가정을 끄니 컴파일러는 실제로 `edi`(=`x`)를 `INT_MAX`와 비교하는 명령을 만들어야 했다. 이 실측이 말해 주는 것은 하나다. **"오버플로가 나도 검사로 잡으면 된다"는 생각 자체가 성립하지 않는다** — 그 검사식이 오버플로를 일으키는 연산을 포함하는 순간, 검사식 자신이 UB의 가정 아래서 최적화돼 사라질 수 있다. 오버플로는 일어난 **뒤에** 잡는 게 아니라 일어나기 **전에** 막아야 한다(범위를 먼저 확인하고 나서 연산하는 순서로).

::: deep 이 가정이 성능 자산인 이유
`-fstrict-overflow`(기본 켜짐)가 없으면 컴파일러는 `for (int i = 0; i < n; i += step)` 같은 루프에서 `i`가 오버플로하지 않는다고 증명할 수 없어 루프 경계 추론, 벡터화, 강도 감소(strength reduction) 같은 최적화를 포기해야 한다. UB로 지정된 덕분에 컴파일러는 "오버플로하는 입력은 프로그램에 들어오지 않는다"고 가정하고 훨씬 공격적으로 최적화한다. 대가는 정확히 이 절에서 본 것 — 그 가정이 깨지는 입력이 실제로 들어오면 검사 코드 자체가 증발한다.
:::
...
**AddressSanitizer(`-fsanitize=address`)**는 메모리 **접근 범위**를 감시한다. 잡는 것: 힙/스택/전역 버퍼의 범위 밖 접근, use-after-free, use-after-scope, 이중 해제, 일부 메모리 누수. 위 표의 앞 다섯 줄이 전부 ASan의 영역이다. 못 잡는 것: 접근 자체는 유효한 메모리 안에서 일어나는 버그 — signed 오버플로는 계산 결과가 여전히 `int` 한 칸 안에 있으니 ASan이 볼 이유가 없고, [2.6 복사 시맨틱](#/copy-semantics)의 자기 대입 버그처럼 "유효한 메모리를 잘못된 순서로 읽는" 논리 오류도 ASan의 감시 범위 밖이다.

**UndefinedBehaviorSanitizer(`-fsanitize=undefined`)**는 반대로 언어 차원의 **연산 규칙 위반**을 감시한다. 잡는 것: signed/unsigned 오버플로, 널 포인터 역참조, 정렬 위반, 0으로 나누기, 잘못된 시프트 폭, 유효하지 않은 enum/bool 값으로의 캐스팅. 못 잡는 것: 메모리 범위 자체는 UBSan의 관심사가 아니다 — `arr[10]`이 크기 4짜리 배열을 넘어서도, 그 인덱스가 컴파일 타임에 안 보이면 UBSan은 조용하다(이건 ASan의 일이다).

두 도구 모두 **초기화 안 된 값 읽기는 못 잡는다.** 그 진단은 MemorySanitizer(MSan)의 몫인데, MSan은 clang 전용이고 링크되는 모든 코드(표준 라이브러리 포함)가 같은 방식으로 계측돼 있어야 해서 g++ 기반 프로젝트에서는 사실상 못 쓴다. [1.3](#/variables)에서 미초기화 읽기를 `-Wmaybe-uninitialized`라는 **컴파일 타임 경고**로만 잡을 수 있었던 이유가 이것이다 — 런타임 새니타이저가 이 부류를 통째로 놓치기 때문에, 정적 분석 쪽 경고에 더 의존해야 한다.

### 동시 사용법

두 도구는 서로 다른 계측이라 같은 빌드에 함께 넣을 수 있다.
...
첫 줄은 UBSan이 6번째 줄의 오버플로를 진단하고 지나간 것이다(UBSan은 기본적으로 진단만 찍고 실행을 이어간다). 그 뒤 ASan이 10번째 줄의 힙 범위 밖 쓰기에서 프로세스를 즉시 멈춘다. **한 실행 안에 서로 다른 두 등급의 버그가 각자의 도구로 각자 잡혔다** — 이게 `address,undefined`를 같이 켜는 이유다.
...
**정적 분석을 앞단에 둔다.** 새니타이저는 그 코드 경로가 실제로 실행돼야 잡는다 — 테스트가 안 지나간 경로의 UB는 새니타이저도 못 본다. 실행 없이 코드를 훑어 의심 패턴을 잡는 정적 분석기가 이 구멍을 메운다. `clang-tidy`는 [7.7 clang-tidy와 정적 분석](#/static-analysis)에서 본격적으로 다룬다.

**테스트는 항상 새니타이저 빌드로 돌린다.** 일반 빌드로 통과한 테스트가 안전하다는 뜻이 아니다 — 이 절과 앞선 여러 절에서 본 것처럼 UB는 "우연히 멀쩡해 보이는" 결과를 자주 낸다. CMake 프리셋에 `-fsanitize=address,undefined`를 켠 디버그 구성을 하나 따로 두고, 로컬 테스트와 CI 둘 다 그 구성으로 돌리는 것이 관례다.

**경고를 에러로 취급한다.** `-Wall -Wextra`가 잡아 주는 UB의 전조(미초기화 사용 경고, 댕글링 포인터 경고, 부호 비교 경고)를 팀 전체가 무시하지 않게 만드는 유일한 방법은 `-Werror`다. 경고 하나를 빌드 실패로 만들면, "나중에 고치겠다"는 선택지 자체가 없어진다 — [0.3](#/first-build)에서 "경고 0개"를 기준으로 잡은 이유가 여기서 완성된다.

## 왜 로봇 코드에서 UB가 유독 위험한가

UB의 가장 나쁜 성질은 "안 보인다"가 아니라 **"보이는지 여부가 빌드 설정에 달렸다"**는 것이다. 실측으로 확인한다. 분기 하나에서만 초기화되는 지역 변수를 반환하는 함수다.
...
1. 다음 세 가지를 정의됨/미지정/구현 정의/미정의 중 하나로 분류하고 각각 근거를 한 문장씩 써라. ① `long`의 크기가 Linux(LP64)와 Windows(LLP64)에서 다른 것 ② `unsigned int u = 0; u - 1;`의 결과 ③ `int* p; *p;` (초기화 안 된 포인터 역참조).
2. 이 절의 `branch_erase.cpp`에서 `-fwrapv` 없이 컴파일하면 `if (x + 1 > x)`가 항상 참으로 접힌다. 이 최적화가 성립하려면 컴파일러가 어떤 가정을 세워야 하는지, 그 가정이 왜 "오버플로 이후에 검사하는" 방어 코드를 무력화하는지 설명하라.
3. (실습) `uninit_fold.cpp`를 그대로 쳐서 `-O0`과 `-O2`로 각각 빌드하고 실행하라. 그다음 `objdump -d --disassemble=_Z7computei`로 `-O2` 결과물의 `compute` 함수를 직접 디스어셈블해, `mode` 파라미터가 실제로 쓰이는지 확인하라. 성공 기준: 두 빌드의 출력이 다르고, `-O2` 어셈블리에 `mode`(`%edi`)를 읽는 명령이 없다.
4. (실습) 새니타이저 두 개 동시 검출: 한 함수 안에 signed 오버플로(예: `INT_MAX + 1`)와 힙 버퍼 오버플로(예: `new int[4]` 뒤 `arr[4]` 쓰기)를 순서대로 넣은 프로그램을 작성하라. `g++ -std=c++20 -Wall -Wextra -g -fsanitize=address,undefined`로 빌드해 실행하고, 출력에서 ① UBSan의 `runtime error: signed integer overflow` 줄과 ② ASan의 `heap-buffer-overflow` `SUMMARY` 줄을 각각 찾아 어느 줄 번호에서 났는지 말하라. 성공 기준: 두 진단이 한 실행 안에서 순서대로 나타난다.
5. `-fsanitize=address,undefined`를 배포되는 로봇 바이너리에 상시로 켜두지 않는 이유를 이 절의 실측 두 가지(오버헤드, 프로세스 종료 방식)를 근거로 설명하라.
:::
