...
이 과정을 **템플릿 인스턴스화(template instantiation)** 라 부른다. 함수 템플릿 자체는 실행 코드가 아니라 컴파일러에게 주는 설계도이고, 실제 코드는 어떤 타입으로 호출되는 순간 그 타입을 `T` 자리에 치환해 컴파일러가 "찍어낸다." 심볼에 붙은 `W`(weak) 표시도 단서다 — 인스턴스화된 템플릿 함수는 여러 번역 단위에서 중복 정의돼도 괜찮은 약한 심볼로 나온다. 헤더 하나를 여러 `.cpp`가 include했을 때 같은 인스턴스가 중복 생성되는 문제를 링커가 접는 방식은 [1.10 네임스페이스와 링크리지](#/linkage)에서 본 `inline`의 ODR 예외와 정확히 같다 — 컴파일러가 템플릿 인스턴스에 자동으로 `inline` 취급을 준다고 이해하면 된다.

이 과정 — 같은 설계도가 호출 타입마다 별도 함수로 찍혀 나오는 순서 — 을 스텝 단위로 재생하며 확인한다.
...
(g++ 13.3 실측 — 경고 없이 링크까지 끝난다.) 이제 `main_fixed.cpp`가 `max_val_fixed.h`를 include하는 순간, 그 번역 단위 안에 `max_val`의 **본문**과 `max_val(3, 5)`라는 **호출**이 동시에 존재한다. 컴파일러는 그 자리에서 `T = int`를 추론하고, 본문을 보고 있으니 바로 인스턴스화해 `max_val<int>(int, int)`를 `main_fixed.o` 안에 직접 찍어 넣는다. **템플릿을 인스턴스화하려면 컴파일러가 호출 시점에 본문 전체를 봐야 한다** — 이것이 함수 템플릿·클래스 템플릿을 관례적으로 헤더에 통째로 넣는 이유의 전부다.
...
1. 오버로딩과 함수 템플릿이 "타입마다 다른 함수가 필요하다"는 점에서는 같은데도, 소스 코드 유지보수 관점에서 근본적으로 다른 이유를 설명하라. 오버로드 해석과 템플릿 인자 추론 중 어느 쪽이 암묵적 타입 변환에 더 관대한가?

2. `deduce_fail.cpp`의 에러 메시지 마지막 줄 `deduced conflicting types for parameter 'T' ('int' and 'double')`가 정확히 무엇을 말하는지, `T`가 파라미터 두 개에 각각 어떻게 추론됐는지를 근거로 설명하라.

3. 함수 템플릿의 정의를 헤더가 아니라 `.cpp`에만 두고, 그 템플릿을 호출하는 코드를 같은 `.cpp` 안에 둔다면(다른 파일에서 include하지 않는다면) 링크가 성공할지 실패할지 예측하고 이유를 대라.

4. (실습) 이 절의 `overloads.cpp`를 그대로 타이핑해 실행한 뒤, 넷째 타입으로 `long`을 처리하는 오버로드를 손으로 추가해 봐라. 그다음 같은 기능을 함수 템플릿 하나로 다시 짜서 `long`, `int`, `double`, `std::string` 네 타입 모두를 오버로드 추가 없이 호출해 봐라. 성공 기준: `g++ -std=c++20 -Wall -Wextra`로 경고 없이 컴파일되고, `nm -C`로 확인했을 때 인스턴스가 정확히 4개 나온다.

5. (실습) `deduce_fail.cpp`를 그대로 타이핑해 컴파일 에러를 재현하고 `note` 세 줄을 전부 읽어라. 그다음 ① `max_val<double>(...)`로 명시적 지정 ② 두 번째 인자를 `static_cast<int>(3.5)`로 캐스팅 — 두 가지 방법으로 각각 컴파일을 통과시켜 보고, 두 해법이 최종적으로 계산에 쓰는 값이 서로 다르다는 것을 출력으로 확인하라.
:::

::: answer 해설
1. 오버로딩은 타입이 하나 늘 때마다 개발자가 함수 본문을 다시 작성해야 하고, 그 본문들은 컴파일러 입장에서 서로 아무 관계가 없는 별개의 함수다 — 버그를 고치려면 오버로드 개수만큼 고쳐야 한다. 템플릿은 본문이 물리적으로 하나뿐이라 고칠 곳도 하나다. 암묵 변환은 오버로드 해석이 더 관대하다 — 오버로드 해석은 정확 일치/승격/변환 세 단계를 다 고려하지만, 템플릿 추론은 정확 일치만 보고 승격조차 해 주지 않는다(`char`가 `int`로 승격되는 오버로딩과 달리, 템플릿은 서로 다른 타입이 오면 바로 추론 실패다).
2. 첫 인자 `3`은 `int`이므로 `T = int`가, 둘째 인자 `3.5`는 `double`이므로 `T = double`이 각각 추론된다. 그런데 시그니처 `T max_val(T a, T b)`는 두 파라미터가 **같은** `T`를 요구하므로, 서로 다른 두 후보(`int`와 `double`)가 나온 시점에 추론 자체가 실패로 처리된다.
3. 성공한다. 템플릿이 헤더에 있어야 하는 이유는 "호출 시점에 본문과 타입 정보가 같은 번역 단위 안에 함께 있어야 한다"는 것이지, 파일 확장자가 `.h`여야 한다는 규칙이 아니다. 정의와 호출이 같은 `.cpp` 하나 안에 있으면 그 번역 단위 안에서 컴파일러가 인스턴스화를 끝낼 수 있다. 다만 그 함수를 다른 `.cpp`에서도 쓰려면 그 순간부터는 헤더로 옮겨야 앞서 본 `undefined reference`를 피한다.
4. `long` 오버로드는 `long max_val(long a, long b) { return a > b ? a : b; }`처럼 셋과 같은 본문을 한 번 더 베끼는 형태가 된다. 템플릿 버전은 `overloads.cpp`의 함수 네 개를 지우고 `tmpl_basic.cpp`의 `template <typename T> T max_val(T a, T b) {...}` 하나로 교체하는 것으로 끝난다 — `nm -C 산출물.o | grep -c max_val`이 4를 보여주면 성공이다.
5. `note`는 순서대로 후보 시그니처, 추론/치환 실패 지점, 충돌한 두 타입을 알려 준다. `max_val<double>(3, 3.5)`는 `3`을 `3.0`으로 바꿔 `3.5`와 정상 비교해 `3.5`를 낸다. `max_val(static_cast<int>(3.5), 3)` 즉 `max_val(3, 3)`처럼 캐스팅으로 통일하면 `3`이 나온다 — 두 해법이 서로 다른 값(`3.5` vs `3`)을 내는 것은 "타입을 무엇으로 통일했는가"에 따라 원래 정보(소수부)를 보존하느냐 버리느냐가 갈리기 때문이다.
:::

이 절의 코드는 전부 네 IDE에서 직접 쳐라. 특히 `tmpl_basic.cpp`를 컴파일한 뒤 `nm -C 산출물.o | grep max_val`을 직접 돌려서 심볼이 몇 개 나오는지 눈으로 세어 봐라 — 위젯에서 본 "찍혀 나오는" 과정이 추상적 비유가 아니라는 것이 이 한 줄로 확인된다. `max_val.h`/`max_val.cpp`/`main_split.cpp` 세 파일은 그대로 분리 컴파일해 링크 에러를 먼저 재현한 다음, 정의를 헤더로 옮겨 고쳐라. 기준 명령: `g++ -std=c++20 -Wall -Wextra main.cpp -o main && ./main`, 분리 컴파일은 `g++ -std=c++20 -Wall -Wextra -c a.cpp -o a.o && g++ -std=c++20 -Wall -Wextra -c b.cpp -o b.o && g++ a.o b.o -o main`.
